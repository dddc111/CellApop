...
class SeqConv3x3(nn.Module):
    def __init__(self, seq_type, inp_planes, out_planes, depth_multiplier):
        ...
        if self.type == 'conv1x1-conv3x3':
            ...
            conv0 = nn.Conv2d(self.inp_planes, self.mid_planes, kernel_size=1, stride=1, padding=0)
            self.k0 = conv0.weight
            self.b0 = conv0.bias

            conv1 = nn.Conv2d(self.mid_planes, self.out_planes, kernel_size=3, stride=1, padding=1)
            self.k1 = conv1.weight
            self.b1 = conv1.bias
            
        elif self.type == 'conv1x1-sobelx':
            conv0 = nn.Conv2d(self.inp_planes, self.out_planes, kernel_size=1, stride=1, padding=0)
            self.k0 = conv0.weight
            self.b0 = conv0.bias
    ...
    def forward(self, x):
        if self.type == 'conv1x1-conv3x3':
            ...
            b0_pad = self.b0.view(1, -1, 1, 1) # B C H W
            y0[:, :, 0:1, :] = b0_pad
            y0[:, :, -1:, :] = b0_pad
            y0[:, :, :, 0:1] = b0_pad
            y0[:, :, :, -1:] = b0_pad
...
class ECB(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.permute:
            y = y.permute(0, 2, 3, 1) # [B, C, H, W] --> [B, H, W, C]
            y = self.norm(y)
...
class EoS(nn.Module):
    ...
    def rep_params(self):
        K0, B0 = self.conv3x3.weight, self.conv3x3.bias
        K1, B1 = self.conv1x1_sbx.rep_params()
        K2, B2 = self.conv1x1_sby.rep_params()
        RK, RB = (K0+K1+K2), (B0+B1+B2)
...
class EoL(nn.Module):
    ...
    def rep_params(self):
        K0, B0 = self.conv3x3.weight, self.conv3x3.bias
        K1, B1 = self.conv1x1_lpl.rep_params()
        RK, RB = (K0+K1), (B0+B1)
