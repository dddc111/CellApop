...
from .token_mixer import EoS, EoL
...
class TokenMixer(nn.Module):
    """实现图中所示的token mixer结构"""
    def __init__(self, inp, oup, use_se=True, stride=1):  # 添加stride参数
        ...
        # 1×1 DW 分支
        self.dw1x1_1 = Conv2d_BN(inp, inp, 1, 1, 0, groups=inp)
        self.dw1x1_2 = Conv2d_BN(inp, inp, 1, 1, 0, groups=inp)
        self.dw1x1_3 = Conv2d_BN(inp, inp, 1, 1, 0, groups=inp)
        
        # 3×3 DW 分支
        self.dw3x3 = Conv2d_BN(inp, inp, 3, 1, 1, groups=inp)
        
        # 边缘检测操作
        self.sobel_x = EoS(inp, permute=False)  # Sobel-x
        self.laplacian = EoL(inp, permute=False)  # Laplacian
        self.sobel_y = EoS(inp, permute=False)  # Sobel-y
        
        # SE模块
        self.se = SqueezeExcite(inp, 0.25) if use_se else nn.Identity()
        
        # 1×1卷积调整通道
        self.proj = Conv2d_BN(inp, oup, ks=1, stride=1, pad=0)
...
# 然后修改RepViTBlock类中的token_mixer实现
class RepViTBlock(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_hs, skip_downsample=False):
        ...
        if stride == 2:
            ...
            # self.token_mixer = TokenMixer(inp, oup, use_se)
